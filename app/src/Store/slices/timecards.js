...
const slice = createSlice({
  name: "timecards",
  initialState: [
    { id: 0, cardId: "", userId: "", projectId: "", startTime: "", hours: "" },
  ],
  reducers: {
    // actions => action handlers
    timecardsReceived: (timecards, action) => {
      //TODO seperate action for receiveing timecards after adding one
        ...
        if (isValidSource)
          ...
          items.push({
            ...
            startTime: action.payload.startTime
              ? resultDate(action.payload.startTime)
              : "2000-01-01T00:00:00.000Z",
            endTime: action.payload.endTime
              ? resultDate(action.payload.endTime)
              : "2000-01-01T00:00:00.000Z",
      ...
      items.forEach((timecard) => {
        ...
        if (notADuplicate()) timecards.push(timecard);
      });
    },
    ...
    timecardRenamed: (timecards, action) => {
      const selectedCard = timecards.find(
...
export const {
  timecardAdded,
  timecardRemoved,
  timecardsOfUserRemoved,
  timecardsOfProjectRemoved,
  timecardProjectChanged,
  timecardRenamed,
  timecardWorkingTimeEdited,
  timecardsReceived,
  timecardsReset,
  error,
} = slice.actions;
...
export const loadTimecards = (session, urlExtension) =>
  apiCallBegan({
...
export const loadUserTimecards = (session) =>
  apiCallBegan({
    url: url2,
    headers: {
      session,
...
export const postUserTimecard = (session, urlExtension, data) =>
  apiCallBegan({
...
export const editTimecard = (session, urlExtension, data) =>
  apiCallBegan({
    url: `${url}/${urlExtension}`,
    method: "PATCH",
    data,
...
export const deleteTimecard = (session, urlExtension) =>
  apiCallBegan({
    url: `${url}/${urlExtension}`,
    method: "DELETE",
    headers: {
